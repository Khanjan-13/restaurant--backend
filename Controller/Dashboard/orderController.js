...
const getAllOrders = async (req, res) => {
  try {
    ...
    // Determine the creator ID (use adminId if staff, otherwise use userId)
    const creatorId = adminId || userId;

    // Fetch orders created by the authenticated user/admin
    const ordersData = await Orders.find({ createdBy: creatorId })
      .populate('tableId', 'tableNumber section') // Populate table details
    ...
    if (ordersData.length === 0) {
      console.log("No orders found for user:", creatorId);
    ...
    const enhancedOrders = ordersData.map(order => ({
      ...
      customerPhone: order.customerId?.phone || order.customerPhone || 'N/A',
      tableNumber: order.tableId?.tableNumber || order.tableNumber || 'N/A',
      tableSection: order.tableId?.section || ''
    }));
